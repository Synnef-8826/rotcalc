...
argList = [ # short,long,desc,type,options?
        ...
        ["-m", "--manual", "run rotcalc in manual mode with the current dataset, meant to quickly preview leveling progress or individual rotations", "store_true"],
        ["-o", "--output", "send all output to file OUTPUT", "store"],
        ["-s", "--settings", "print current settings", "store_true"],

        ["-d", "--dataset", "change the active dataset (must be a filename in ./data/datasets, e.g 'example.json')", "store"],
...
if args.manual:
        from calculator import manualPlotting
        manualPlotting()
